...
{	
	...
	{
		echo "<table>";
		echo'<tr><td colspan="10" style="font-weight:bold;">Liste des caissiers</td></tr>';
		...
		$req=$bd->query("SELECT * FROM gerant");
		$reqt=$bd->query("SELECT * FROM gerant");
		if($reqt=$reqt->fetch())
		{
			echo "<table>";
			echo'<tr><td colspan="10" style="font-weight:bold;">Liste des gérants</td></tr>';
			echo"<tr><th>Num id</th><th>Prenom</th><th>Nom</th><th>Sexe</th><th>adresse</th><th>Email</th><th>Tel</th><th>Age</th></tr>";
			while($re=$req->fetch())
				echo"<tr><td>".$re['num_i_ng']."</td><td>".$re['prenom']."</td><td>".$re['nom']."</td><td>".$re['sexe']."</td><td>".$re['adresse']."</td><td>".$re['email']."</td><td>".$re['tel']."</td><td>".$re['age']."</td></tr>";
			echo "</table>";
		}	
	} 
	else
			...
			{	$r=$bd->query("SELECT* FROM caissier WHERE groupe like '$nom_g'");
				if($r)
				{
					...
					{	
						...
						echo "<span>Le $nom_g n'a pas de membres pour le moment</span>";
				} 
				else
