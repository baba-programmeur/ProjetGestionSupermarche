    ...
    if($bd)
	{
	    ...
	    if($sta=="selon_produit")
		{
		    ...
		    if($res['nb_prod_vendu']!=0)
		    {
		        ...
		        $r=$bd->query("SELECT DISTINCT nom_prod_v FROM ventes");
				$req=$bd->query("SELECT DISTINCT nom_prod_v FROM ventes");
				$req=$req->fetch();
		        if($req)
		        { 
			        $i=0;
			        ...
			        echo "<span>Aucune vente n'a été effectuée</span><br/>";
		    }
		    else
			    echo "<span>Aucune vente n'a été effectuée</span>";
	    }
		 //Partie statistiques selon les groupes//
		else if($sta=="selon_groupe")
		{
		    $r=$bd->query("SELECT DISTINCT groupe FROM ventes");
		    $req1=$bd->query("SELECT DISTINCT groupe FROM ventes");
			$r1=$req1->fetch();
		    if($r1)
		    { 
			    $i=0;
		...
		//Partie statistiques selon periode//
		    ...
		    header("Location:selon_periode_p.php");
	}	
    else
